...
<body class="bg-[#f8fafc] text-slate-800">
    <div class="flex h-screen overflow-hidden">
        ...
        <aside class="w-[280px] bg-emerald-950 text-emerald-50 h-screen sticky top-0 flex flex-col shrink-0 z-50">
            ...
            <nav class="flex-1 py-8 px-4 space-y-1 overflow-y-auto">
                ...
                <div class="pt-5 pb-2 px-2">
                    <span class="text-[11px] font-bold uppercase tracking-widest text-slate-500/70">Manajemen</span>
                </div>

                <a href="{{ route('dokter.jadwal') }}" class="flex items-center gap-4 px-4 py-4 rounded-xl {{ request()->routeIs('dokter.jadwal') || request()->routeIs('dokter.jadwal.buat-rekam') ? 'sidebar-active' : 'sidebar-link' }}">
                ...
                <a href="{{ route('dokter.pasien') }}" class="flex items-center gap-4 px-4 py-4 rounded-xl {{ request()->routeIs('dokter.pasien') || request()->routeIs('dokter.rekam.riwayat') ? 'sidebar-active' : 'sidebar-link' }}">
                    <i class="fa-solid fa-user-injured w-5 text-center"></i> Pasien Saya
                </a>

                <a href="{{ route('dokter.rekam-medis') }}" class="flex items-center gap-4 px-4 py-4 rounded-xl {{ request()->routeIs('dokter.rekam-medis') || request()->routeIs('dokter.rekam.show') ? 'sidebar-active' : 'sidebar-link' }}">
                    <i class="fa-solid fa-notes-medical w-5 text-center"></i> Rekam Medis
                </a>
            </nav>
        </aside>
        ...
        <div class="flex-1 flex flex-col min-w-0 h-full overflow-hidden">
                ...
                <div class="relative group">
                    <div class="flex items-center gap-3 cursor-pointer">
                        <div class="text-right">
                            <p class="text-[14px] font-bold text-white group-hover:text-emerald-50 transition">
                                {{ auth()->user()->nama ?? 'Dokter' }}
                            </p>
                            <p class="text-[12px] text-emerald-900/70 font-semibold uppercase tracking-wide">
                                Dokter
                            </p>
                        </div>
            ...
            {{-- LOGOUT FORM — wajib ada agar logout berfungsi --}}
            ...
            <main class="flex-1 overflow-y-auto p-8 md:p-12">
                <div class="max-w-7xl mx-auto">
                    @if(session('success'))
                        <div class="mb-6 px-6 py-4 bg-emerald-50 border border-emerald-200 text-emerald-700 rounded-2xl font-semibold text-sm flex items-center gap-3">
                    ...
                    @if(session('error'))
                        <div class="mb-6 px-6 py-4 bg-red-50 border border-red-200 text-red-700 rounded-2xl font-semibold text-sm flex items-center gap-3">
