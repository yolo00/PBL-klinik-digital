...
{{-- Search --}}
<div class="mb-5">
    <form method="GET" action="{{ route('dokter.rekam.riwayat', $pasien->id) }}"
        ...
        @forelse($rekamMedis as $rm)
            <div class="bg-white border border-slate-100 rounded-2xl shadow-sm p-4 mb-3 hover:shadow-lg transition-all duration-300">
                <div class="flex items-start justify-between gap-4">
                    <div>
                        <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">ID Rekam</p>
                        <p class="text-slate-700 font-semibold mt-1">#{{ $rm->id }}</p>

                        <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-3">Tanggal Kunjungan</p>
                        ...
                        <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-3">Diagnosa</p>
                        <p class="text-slate-600 mt-1 break-words">
                            {{ \Illuminate\Support\Str::limit($rm->diagnosa ?? '-', 80) }}
                        </p>
                    </div>
                </div>

                <div class="mt-4 flex flex-col sm:flex-row sm:items-center gap-2 sm:justify-end">
                    <a href="{{ route('dokter.rekam.show', $rm->id) }}"
                       class="inline-flex items-center justify-center gap-1 px-4 py-2 bg-slate-100 text-slate-600 rounded-xl text-xs font-semibold hover:bg-slate-200 transition-all">
                        <i class="fa-solid fa-eye text-[10px]"></i> Lihat
                    </a>
                    <a href="{{ route('dokter.rekam.export-pdf', $rm->id) }}"
                       class="inline-flex items-center justify-center gap-1 px-4 py-2 bg-blue-50 text-blue-600 rounded-xl text-xs font-semibold hover:bg-blue-100 transition-all">
                        <i class="fa-solid fa-file-pdf text-[10px]"></i> PDF
                    </a>
                </div>
            </div>
        ...
        <div class="px-6 py-4 border-t border-slate-50 flex items-center justify-between">
            ...
            <div class="flex items-center gap-1">
                ...
                @foreach($rekamMedis->getUrlRange(1, $rekamMedis->lastPage()) as $page => $url)
                    ...
                    @else
                        <a href="{{ $url }}"
                           class="px-3 py-1.5 text-slate-600 bg-white rounded-lg text-xs font-semibold hover:bg-blue-50 hover:text-blue-600 transition-all border border-slate-200">{{ $page }}</a>
                    @endif
                @endforeach
