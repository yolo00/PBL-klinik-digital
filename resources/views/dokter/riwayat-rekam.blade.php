...
{{-- Search --}}
    <form method="GET" action="{{ route('dokter.rekam-medis') }}"
          class="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 w-full md:w-auto">
        ...
        @forelse($rekamMedis as $rm)
            <div class="bg-white border border-slate-100 rounded-2xl shadow-sm p-4 mb-3 hover:shadow-lg transition-all duration-300">
                <div class="flex items-start justify-between gap-4">
                    <div>
                        <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">ID</p>
                        <p class="text-slate-700 font-semibold mt-1">#{{ $rm->id }}</p>

                        <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-3">Tanggal</p>
                        ...
                        <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-3">Nama Pasien</p>
                        <p class="text-slate-800 font-semibold mt-1 break-words">
                            {{ $rm->jadwal->pasien->user->nama ?? 'Pasien Tidak Ditemukan' }}
                        </p>
                        ...
                        <p class="text-slate-600 mt-1 break-words">
                            {{ \Illuminate\Support\Str::limit($rm->diagnosa ?? '-', 120) }}
                        </p>
                    </div>
                </div>
