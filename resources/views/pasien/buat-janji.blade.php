...
@section('content')
    ...
    <section class="mb-10">
        ...
        @if(isset($pendingCount) && $pendingCount > 0)
            @if($pendingCount < 5)
                <div class="mt-4 p-4 bg-yellow-100 text-yellow-800 rounded-2xl border border-yellow-200">
                    <span class="text-xl mr-2">⚠️</span>
                    Anda sudah membuat/belum membayar {{ $pendingCount }} jadwal
                </div>
            @else
                <div class="mt-4 p-4 bg-red-100 text-red-800 rounded-2xl border border-red-200">
                    <span class="text-xl mr-2">🛑</span>
                    Tolong laksanakan konsultsi jadwal janji temu/lunasi pembayaran yang telah dibuat terlebih dahulu
                </div>
            @endif
        @endif
    </section>
    ...
    <form action="{{ route('pasien.store_jadwal') }}" method="POST" class="bg-white p-10 rounded-[2.5rem] shadow-sm border border-gray-100 space-y-10">
        ...
        <div class="pt-10 border-t border-gray-100 flex flex-col md:flex-row justify-between items-center bg-gray-50 p-8 rounded-3xl gap-6">
            ...
            <button type="submit"
                    class="w-full md:w-auto bg-emerald-600 text-white px-12 py-5 rounded-2xl font-bold text-lg hover:bg-emerald-700 transition-all shadow-xl shadow-emerald-100 flex items-center justify-center gap-3 disabled:bg-gray-400 disabled:cursor-not-allowed disabled:shadow-none"
                    {{ (isset($pendingCount) && $pendingCount >= 5) ? 'disabled' : '' }}>
                📅 Daftar Jadwal Sekarang
            </button>
        </div>
    </form>

<x-buat-janji-scripts />
@endsection